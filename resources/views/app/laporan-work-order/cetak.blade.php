<x-pdf-layout :title="$title">
    <main>
        ...
        <div class="w-full text-left" style="padding:3px; margin-top:5px">
            <span class="d-block uppercase fw-bold" style="font-size: 14px">status: {{ $status }}</span>
        </div>
                    ...
                    @foreach ($tasks as $task)
                        <tr class="borderbottom">
                            <td colspan="6" class="borderleft borderright">
                                ...
                                {{ tanggal($task->deadline, 'd F Y H:i') }} ({{ $task->penerima_tugas->name }} -
                                {{ $task->penerima_tugas->role->name }})
                            </td>
                        </tr>
